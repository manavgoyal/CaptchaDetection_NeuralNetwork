...
class Graph(object):
    ...
    def get_inputs(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.input_mapping[node]
    ...
    def get_output(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.output_mapping[node]
    ...
    def get_gradient(self, node):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.gradient_mapping[node]
    ...
    def add(self, node):
        ...
        self.input_mapping[node] = inputs
        self.nodes.append(node)

        self.output_mapping[node] =  node.forward(self.get_inputs(node))

        self.gradient_mapping[node] = np.zeros_like(self.get_output(node)) 
    ...
    def backprop(self):
        ...
        self.gradient_mapping[loss_node] += 1.0
        i = 0
        for nodes in reversed(self.nodes):
            # if(nodes == loss_node):
            #     self.gradient_mapping[nodes]+= nodes.backward(self.get_inputs(nodes), 1.0)
                
            # else:
            parents = nodes.get_parents()
            for i in range(len(parents)):
                x = parents[i]
                self.gradient_mapping[x]+= nodes.backward(self.get_inputs(nodes), self.gradient_mapping[nodes])[i]
    ...
    def step(self, step_size):
        ...
        for nodes in self.nodes:
            if(type(nodes) == Variable):
                nodes.data -= self.get_gradient(nodes)* step_size
...
class MatrixMultiply(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        part1 = np.dot(gradient, inputs[1].T)
        part2 = np.dot(inputs[0].T, gradient)
        return [part1, part2]
...
class MatrixVectorAdd(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        part2 = np.sum(gradient,0)
        return [gradient, part2]
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [np.array(gradient * np.where(inputs[0] >0, 1 , 0))]
...
class SquareLoss(FunctionNode):
    ...
    def backward(inputs, gradient):
        ...
        part1  = gradient * 1.0/len(inputs[0]) * sub
        part2  = gradient * 1.0/len(inputs[0]) * -sub
        return [part1, part2]
